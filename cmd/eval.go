...
func init() {
	...
	evalCmd.Flags().IntVar(&evalChunkSize, "chunk-size", defaultChunkSize, "chunk size in characters")
	evalCmd.Flags().IntVar(&evalOverlap, "overlap", defaultOverlap, "overlap between chunks in characters")
...
func runEval(cmd *cobra.Command, _ []string) error {
	...
	log.Info().
		...
		Int("chunkSize", evalChunkSize).
		Int("top_k", evalTopK).
		Str("work_dir", evalWorkDir).
		Bool("reuse_db", evalReuseDB).
		Str("embed_model", evalEmbedModel).
		Str("embed_digest", digest).
		Msg("starting eval")
	...
	if err != nil {
		if errors.Is(err, context.Canceled) || errors.Is(err, context.DeadlineExceeded) {
			log.Warn().Err(err).Int("completed", len(report.Queries)).Msg("run interrupted, writing partial report")
		} else {
			return fmt.Errorf("run: %w", err)
		}
	}
	...
	outputDest := evalOutput
	if outputDest == "" {
		outputDest = "stdout"
	}
	log.Debug().Str("output", outputDest).Str("format", evalFormat).Msg("writing report")
