...
A production-grade RAG pipeline in Go. Embed a document corpus into a local vector store, retrieve the most relevant context, and stream answers from any OpenAI-compatible LLM — with pluggable inference backends, an assertion-based retrieval eval harness, and built-in prompt injection mitigation.

Runs fully local with [Ollama](https://ollama.com). Scales to shared-GPU deployments with [vLLM](https://docs.vllm.ai). Query through the CLI, connect it to Claude or any MCP-compatible assistant via the built-in [MCP server](docs/mcp.md), or integrate service-to-service over [gRPC](docs/serve.md) with native token streaming.
